...
class Template {
        ...
        switch ($Path[0]) {
            case "panel":
                    if ($CMS->Session->GetRole() === 3 || $CMS->Session->GetRole() === 2){
                        if (str_contains(implode("/", $Path), '.css') || str_contains(implode("/", $Path), '.js') || str_contains(implode("/", $Path), '.png') || str_contains(implode("/", $Path), '.svg')){
    ...
    private function Load_Page_Template($Path){
        ...
        if ($Permission === null)
            exit();
        if (str_contains($Path, '.css') || str_contains($Path, '.js') || str_contains($Path, '.jpg')  || str_contains($Path, '.jpeg') || str_contains($Path, '.png')  || str_contains($Path, '.svg') || str_contains($Path, '.ico') || str_contains($Path, '.webp') || str_contains($Path, '.mp4')){
        ...
        exit(header('Location: ../404?log=not-found'));
    }
